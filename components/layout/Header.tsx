          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-[68px] lg:h-[76px] flex items-center justify-between gap-4">
            ...
            <Link href={prefix} className="flex items-center gap-3 shrink-0 group" aria-label={site.name}>
              ...
              <span className="flex flex-col leading-none">
                ...
                <span className={cn("text-[9px] sm:text-[10px] font-semibold uppercase tracking-[0.18em] mt-1", transparent ? "text-gold-500" : "text-gold-700")}>
                    ...
                    <div key={col.title.en} className="min-w-0">
                      ...
                      {col.viewAll && (
                        <Link
                          ...
                          className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-brand-700 hover:text-gold-700"
                        >
                          {d.nav.allProducts}
          ...
          <div className="mx-auto max-w-2xl mt-[12vh] px-4">
            <div className="bg-white rounded-xl shadow-dropdown overflow-hidden">
              <div className="flex items-center gap-3 px-5 border-b border-slate-100">
                ...
                />
                <button onClick={() => setSearch(false)} aria-label={d.nav.close} className="p-2 text-slate-500 hover:text-navy-900 cursor-pointer">
              ...
              {flatResults.length > 0 && (
                <div className="flex items-center justify-center gap-4 px-4 py-2 bg-slate-50 border-t border-slate-100 text-[11px] text-slate-500">
...
function Group({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="mb-2">
      <p className="px-2 py-1.5 text-[11px] font-bold uppercase tracking-wider text-slate-500">{label}</p>
      {children}
    </div>
  );
}
